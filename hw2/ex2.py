...
NO_AGGREGATE_RET_TIME = int(RECORDS_IN_A_MB*5*1e3)
AGGREGATE_RET_TIME = int(RECORDS_IN_A_MB*24*60*60*1e3)
...
def is_silence(indata, downsampling_rate, frame_length_in_s, dbFSthres, duration_thres):
    ...
    #print('Non silence frame: ',non_silence_frames)
    non_silence_duration = (non_silence_frames + 1) * frame_length_in_s
    #print('Non silence duration: ',non_silence_duration)p
...
def callback(indata, frames, call_back, status):
    ...
    if not store_audio:
        ...
        if (prediction_magnitude > 0.95 and prediction_label == 'go'):
            store_information = True
        elif (prediction_magnitude > 0.95 and prediction_label == 'stop'):
